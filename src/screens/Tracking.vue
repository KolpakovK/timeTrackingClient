...
export default{
    ...
    methods: {
        findEntries(from="",to=""){
            ...
            axios.get('/api/entries?from='+from+'&to='+to, { headers })
            .then(response => {
        ...
        findTasks(name="",project=""){
            const headers = getHeaders();
            axios.get('/api/tasks?name='+name+'&project='+project+'&limit=5', { headers })
            .then(response => {
        ...
        createRecord(e){
            ...
            axios.post('/api/entries',data, { headers })
            .then(response => {
        ...
        deleteRecord(id){
            const headers = getHeaders();
            axios.delete('/api/entries/'+id, { headers })
            .then(response => {
                const startOfWeek = this.currentDate.clone().startOf('week').format('YYYY-MM-DD');
                const endOfWeek = this.currentDate.clone().endOf('week').format('YYYY-MM-DD');
                this.findEntries(startOfWeek,endOfWeek);
